...
public:
    supplier(int id, const std::string &name, long phn, const std::string &addrLine1, const std::string &city, const std::string &country)
    ...
    friend std::ostream &operator<<(std::ostream &os, const supplier &suppliers) {
        os << "id: " << suppliers.id << " name: " << suppliers.name << " phone: " << suppliers.phn << " addr_line1: "
